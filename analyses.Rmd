...
Il tipo di parto più frequente è quello cesareo, 46% (577), seguito da quello vaginale e quello vaginale operativo, 42% (534) e 12% (154), rispettivamente (fig.2). Il 13% (159) dei soggetti è nato a sequito di fecondazione assistita e il 10% (130) in parto di tipo gemellare.
...
dat_clean %>% 
  ...
  scale_y_discrete(labels = bei_nomi) +
      ...
      labs(
        x = NULL,
        y = NULL, 
        title = " I dieci motivi di consulto piú frequenti"
      )

ggsave(here("figs", "fig_mdc1.png"))
```
...
**Si in generale chi viene per un propblema specifico fa mediamente una visita in piu
 
 
Se e' vero che le percone con parto cesareo hanno in generale piu' problemi, ed e' per quello che vengono piu' spesso, allora dovrebbero esserci piu' soggetti con parto cesareo che non vengono per una visita generale. Questo non sembra supportato dai dati. 
...
dat_year %>% 
  ...
  summarize(n = n(), perc = n/first(tot_g) * 100) %>% 
  
  ggplot(aes(tipo_parto, perc, fill = gruppo)) +
    geom_col(position = "dodge")
```

Quelli che vengono da un parto cesareo hanno mediamente piu visite? No, non sembra proprio che regge.
...
# 
#   ggplot(aes(perc_mdc_1, reorder(mdc_1, perc_mdc_1))) +
#       geom_col(col = "black", position = position_dodge2(), aes(fill = tipo_parto)) +
#       scale_x_continuous(labels = scales::label_percent(scale = 1)) +
#       scale_y_discrete(labels = bei_nomi) +
# 
#       labs(
